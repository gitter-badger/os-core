#!/usr/bin/make -f
# Makefile for TCOS# # (C) Steffen Hoenig <s.hoenig@openthinclient.com> 2013-2015
...
# new
#
# have new busybox 1.23.1
# have some kerberos packages
# kicked out package freerdp 
# ignore /boot/* in base.sfs (excluded in mksquashfs)
# kernel 3.2.0-4-486 is no longer supported
...
BASE_VERSION ?= 2.1
BASE_VERSION_MINOR ?= "-unknown-minor"
...
TARGET_KERNEL = $(TARGET_KERNEL_DEFAULT) $(TARGET_KERNEL_NONPAE) $(TARGET_KERNEL_32)

# run-time packages

TARGET_PACKAGES += alsa-utils aptitude apt-utils arandr ca-certificates cifs-utils console-data console-tools coreutils dbus dbus-x11 dconf-tools devilspie devilspie2 dialog dmidecode dnsutils dos2unix dosfstools e2fsprogs eject ethtool file firmware-linux flashplugin-nonfree fontconfig fxcyberjack gdevilspie gvfs gvfs-backends hdparm htop hwinfo iceweasel iceweasel-l10n-de iproute2 iputils-ping ipython ldap-utils less libcurl3 libdrm2 libdrm-intel1 libdrm-nouveau1a libdrm-radeon1 libgl1-mesa-dri libgl1-mesa-glx libgssglue1 libmotif4 libpam-ldap libsasl2-modules libsasl2-modules-gssapi-mit libssl1.0.0 libstdc++5 libvdpau1 libwebkitgtk-1.0-0 libx11-6 libxerces-c3.1 lightdm lightdm-gtk-greeter locales locales-all lshw ltrace man-db mc mesa-utils net-tools nfs-common ntp numlockx openssh-client openssh-server pavucontrol pciutils pulseaudio pulseaudio-utils pulseaudio-module-x11 python-gconf python-gtk2 python-ldap python-xdg rdesktop rsync screen smplayer spice-client strace sudo syslog-ng tcpdump ttf-dejavu udhcpc usbip usbutils util-linux vim vim-tiny wget x11vnc x11-xserver-utils xdg-utils xfonts-base xinetd xorg xserver-xorg xserver-xorg-core xserver-xorg-input-evdev xserver-xorg-input-multitouch xserver-xorg-input-mutouch xserver-xorg-input-wacom xserver-xorg-video-ati xserver-xorg-video-fbdev xserver-xorg-video-geode xserver-xorg-video-intel xserver-xorg-video-modesetting xserver-xorg-video-nouveau xserver-xorg-video-openchrome xserver-xorg-video-radeon xserver-xorg-video-savage xserver-xorg-video-vesa xtightvncviewer zenity libxdo2 xdotool krb5-user libpam-krb5

# krb5-auth-dialog

TARGET_PACKAGES_BACKPORTS += atril caja engrampa eom glx-alternative-fglrx glx-alternative-nvidia glx-alternative-mesa libfglrx libgl1-nvidia-glx libgl1-nvidia-legacy-173xx-glx mate-applets mate-desktop mate-media mate-media-pulse mate-screensaver mate-session-manager mate-system-monitor mate-themes mate-utils nvidia-alternative nvidia-alternative-legacy-173xx nvidia-driver-bin nvidia-vdpau-driver pluma xserver-xorg-video-nvidia xserver-xorg-video-nvidia-legacy-173xx xvba-va-driver
...
TARGET_PACKAGES_DEB_DKMS_AS_DEP := $(addprefix ./Packages/, $(TARGET_PACKAGES_DEB_DKMS))
...
#justatest:
#	sudo BASE_VERSION=$(BASE_VERSION) BASE_VERSION_MINOR=$(BASE_VERSION_MINOR) Scripts/TCOS.chroot Filesystem $(SHELL) < Scripts/TCOS.tcosify-chroot_test
...
tcosify-stamp:filesystem-stamp busybox-stamp
	...
	sudo BASE_VERSION=$(BASE_VERSION) BASE_VERSION_MINOR=$(BASE_VERSION_MINOR) Scripts/TCOS.chroot Filesystem $(SHELL) < Scripts/TCOS.tcosify-chroot
	@touch $@

update:
	make $@-stamp
update-stamp:tcosify-stamp $(TARGET_PACKAGES_DEB_AS_DEP)
...
driver-stamp:kernel-stamp $(TARGET_PACKAGES_DEB_DKMS_AS_DEP)
...
compressed-stamp:clean-stamp
	...
	nice -10 ionice -c 3 sudo XZ_OPT="-6" mksquashfs Filesystem Base/base-$(BASE_VERSION)/sfs/base.sfs -ef Sources/mksquashfs.excludes -wildcards -noappend -always-use-fragments -comp xz
	@touch $@
...
base-release:
	USER=$(USER) PATH=$(PATH) sudo AUFS=1 BIND_ROOT=./ Scripts/TCOS.chroot Filesystem $(SHELL) -c " \
	    echo \"deb http://packages.openthinclient.org/openthinclient/v2/devel ./\" > /etc/apt/sources.list.d/tcos.list; apt-get update; \
            ...
            cd TCOS/Base/base-$(BASE_VERSION) ;\
	    touch debian/files ;\
	    echo \"Please have your comments to changelog now. Use command: dch -i \" ;\
	    bash ;\
	    tcos build -b ;"
	touch $@-stamp
...
upload-test:
	...
	-rsync Base/base-$(BASE_VERSION)/tftp/*  Base/base-$(BASE_VERSION)/tftp/initrd* $(LOCAL_TEST_PATH)/tftp/
	-rsync Base/base-$(BASE_VERSION)/sfs/*.sfs     $(LOCAL_TEST_PATH)/sfs/
	-rsync -a Base/base-$(BASE_VERSION)/debian/base/custom/*      $(LOCAL_TEST_PATH)/custom/
